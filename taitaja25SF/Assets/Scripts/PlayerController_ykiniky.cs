...
namespace YkinikY
{
    public class PlayerController_ykiniky : MonoBehaviour
    {
          ...
          void Shoot()
        {
            ...
            Vector2 shootDirection = GetComponent<SpriteRenderer>().flipX ? Vector2.left : Vector2.right;
            ...
            // Apply force to the circle in the shooting direction
            Rigidbody2D circleRb = circle.GetComponent<Rigidbody2D>();
            if (circleRb != null)
            {
                circleRb.velocity = shootDirection * shootForce;
            }
        }
        void MovimentUpdate()
        {
